...
def convert_to_assay(df, source_id, output_filepath):
    ...
    df['canonical_smiles'] = mol_objects.apply(lambda x: Chem.MolToSmiles(x) if x is not None else None)
    ...
    for assay_name in assay_names:
        ...
        assay_df['ground_truth'] = assay_df['ground_truth'].astype(int)        # Write each assay to a parquet file
...
@click.argument("input_filepath", type=click.Path(exists=True), help="The file path for the input file. This file should exist.")
@click.argument("output_filepath", type=click.Path(), help="The directory path where the output parquet files will be saved.")
@click.option(
...
def main(input_filepath, output_filepath, input_dataset):
    ...
    smiles_errors, selfies_errors, assay_num = convert_to_assay(df, source_id, output_filepath)
    logger.info(
