...
def make_tasks(input_filepath, output_filepath, support_set_sizes=[16,32,64,128,256]):
    ...
    for assay_filepath in assay_filepaths:
        ...
        dfs = assay_to_tasks(assay_df, support_set_sizes)
        for i, support_set_size, df in dfs:
...
def assay_to_tasks(df, target_support_set_sizes=[16,32,64,128,256], min_query_size=16, num_permutations=3):
    ...
    for support_size in available_support_set_sizes:
        for i in range(num_permutations):
            ...
            dfs.append((i+1, support_size, df.copy()))
    return dfs
